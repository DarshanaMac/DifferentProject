...
public class CommandBase {
    ...
    /**
     * This method is used to Enter event
     *
     * @param element
     */
    @Step("sendKey {0}, Value:{1}")
    protected static void enter(WebElement element) {
        waitHelper.waitForElementDisplay(element);
    ...
    /**
     * This method is used to check web element and text display or not
     *
     * @param element
     */
    @Step
    protected static void isTextDisplay(WebElement element, String actualtext) {
        try {
            String bodyText =element.getText();
            Assert.assertTrue(bodyText.contains(actualtext), "Text not found!");
        } catch (NoSuchElementException e) {
